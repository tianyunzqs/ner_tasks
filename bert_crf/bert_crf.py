...
# 标注数据
train_data, categories = load_data(os.path.join(project_path, 'data/example.train'))
valid_data, _ = load_data(os.path.join(project_path, 'data/example.dev'))
test_data, _ = load_data(os.path.join(project_path, 'data/example.test'))
...
model = build_transformer_model(config_path, checkpoint_path, return_keras_model=False)
